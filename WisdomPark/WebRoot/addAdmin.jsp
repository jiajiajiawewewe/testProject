...
<!--    查询操作 -->
    ...
    <li><label>综合查询</label>
    <input name="  " type="text" class="scinput"  /></li>
  ...
  <% UserDao dao=new UserDao();
    ...
    <div class="formtitle"><span>学生列表</span></div></div>
        ...
        <th>角色</th> <th>邮箱</th> <th>电话</th>
         <th>操作</th> <th>操作</th>
			 ...
			 <table  class="table table-striped"  id="idData"  id="barcon"> 
			   <tbody>
			  <c:forEach items="<%=userlist %>" var="s2">
        <tr>
        <td><input name="" type="checkbox" value="" /></td>
				...
				<td >${s2.password}</td>
				<td><c:if test="${s2.role eq'admin2'}" >信息录入管理员</c:if>
				<c:if test="${s2.role eq'admin'}" >超级管理员</c:if>
				<c:if test="${s2.role eq 'user'}">普通用户</c:if>
				</td>
				<td>${s2.email}</td>
				<td>${s2.phone}</td>
      <td><a href="getoneUSer?name=${s2.name}" class="glyphicon glyphicon-pencil btn-sm" >修改权限</a></td>
				 <td><a href="deleteOneUse?name=${s2.name}" class="glyphicon glyphicon-pencil btn-sm" >删除</a></td>
				 </tr>
       </c:forEach>
